...
class ProvenancedThreshold(BaseModel):
    ...
    """
    metric_name: str = Field(description="Name of the safety metric (e.g. max_storage_temperature)")
    ...
    authority_score: float = Field(default=1.0, ge=0.0, le=1.0, description="Supplier authority weight (0.0 to 1.0)")
    citation: str = Field(description="Human-readable citation string citing the source document and section")
...
class SafetyEvaluationResult(BaseModel):
    ...
    threshold_value: Optional[float] = Field(default=None, description="Retrieved safety threshold value")
    unit: str = Field(description="Measurement unit")
    provenance: Optional[ProvenancedThreshold] = Field(default=None, description="Source provenance metadata")
    reasoning: str = Field(description="Deterministic reasoning summary explaining the verdict")
