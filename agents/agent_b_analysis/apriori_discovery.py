...
KNOWN_INCOMPATIBLE_PAIRS = {
    frozenset(["Nitric Acid", "Ethanol"]): "REACTIVE: Strong oxidizer + organic flammable liquid (Fire/Explosion Risk)",
    frozenset(["Sodium Cyanide", "Sulfuric Acid"]): "TOXIC GAS: Cyanide salt + Strong acid releases Hydrogen Cyanide gas",
    frozenset(["Ammonium Nitrate", "Fuel Oil"]): "EXPLOSIVE: Oxidizer + Fuel mixture (ANFO explosive risk)",
}
...
class CoStoragePatternMiner:
    ...
    def __init__(self, min_support: float = 0.2, min_threshold_lift: float = 1.0) -> None:
    ...
    def discover_co_storage_rules(self, transactions: List[List[str]]) -> List[Dict[str, Any]]:
        ...
        rules = association_rules(frequent_itemsets, metric="lift", min_threshold=self.min_threshold_lift)
        discovered_rules = []
        ...
        for _, row in rules.iterrows():
            ...
            incompatibility_warning = KNOWN_INCOMPATIBLE_PAIRS.get(combined_pair, "COMPATIBLE: No known CAMEO reactivity warning.")
